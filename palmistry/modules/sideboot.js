...
import { drawOverlayDemo } from './ui.js';
import { analyzeMock } from './analyzer.js';
...
export async function boot() {
  ...
  let state = { stream:null, loopStop:null, mirrorRight:true, running:false };

  async function startAll(){
    if (state.running) return;
    state.stream = await openCamera({ facingMode:'environment', width:1920, height:1080 });
    state.loopStop = startLoop({ stream:state.stream, canvasLeft:cL, canvasRight:cR, mirrorRight:()=>state.mirrorRight });
    state.running = true; log('▶️ Camera started');
  }
  function stopAll(){ try{ state.loopStop&&state.loopStop(); }catch{} stopCamera(state.stream); state.running=false; log('🛑 Camera stopped'); }
  function captureRight(){ if(!state.running) return; captureToCanvas(cR, { sourceStream:state.stream, mirror:state.mirrorRight }); log(`📸 Captured (mirror=${state.mirrorRight})`); }
  async function overlay(){ const lines = await analyzeMock(cR); drawOverlayDemo(cR, lines); log('🎨 Overlay drawn'); }
  async function makePDF(){ await exportPDF({ imageDataURL:cR.toDataURL('image/jpeg',0.92), lines:await analyzeMock(cR), meta:{ hand:'Right', mirror:state.mirrorRight, when:new Date().toISOString() } }); log('📄 PDF downloaded'); }
...
function ensureCanvas(id){
  const cv = document.createElement('canvas'); cv.id=id; cv.width=1280; cv.height=720; cv.style.cssText='width:100%;max-height:38vh;border:2px solid #134;border-radius:10px;background:#000;margin-top:6px';
  document.getElementById('sideboot-wrap').querySelector('.cwrap').appendChild(cv); return cv;
}
